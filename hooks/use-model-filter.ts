...
export function useModelFilter(models: Model[]) {
  ...
  const filteredAndSortedModels = useMemo(() => {
    const { sortBy, showReasoningOnly, selectedCompanies, selectedProviders } =
      filterSortState;
  ...
  const { companyList, providerList } = useMemo(() => {
    ...
    models.forEach((model) => {
      if (model.company_name) {
        companyCounts[model.company_name] =
          (companyCounts[model.company_name] || 0) + 1;
      }
      if (model.provider_name) {
        providerCounts[model.provider_name] =
          (providerCounts[model.provider_name] || 0) + 1;
      }
    });
