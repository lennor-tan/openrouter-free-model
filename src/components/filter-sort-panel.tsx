      ...
      <div className="p-4 mb-2 bg-card border rounded-lg shadow-sm">
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    <CommandItem onSelect={() => onCompanyChange('all', true)}>
                      <div className={cn("mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary", selectedCompanies.length === companyList.length ? "bg-primary text-primary-foreground" : "opacity-50 [&_svg]:invisible")}>
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    <CommandItem onSelect={() => onProviderChange('all', true)}>
                      <div className={cn("mr-2 flex h-4 w-4 items-center justify-center rounded-sm border border-primary", selectedProviders.length === providerList.length ? "bg-primary text-primary-foreground" : "opacity-50 [&_svg]:invisible")}>
                        <Check className={cn("h-4 w-4")} />
                      </div>
                      <span>全选</span>
                    </CommandItem>
                    {providerList.map((provider) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span>{provider.name} ({provider.count})</span>
                        </CommandItem>
                      );
                    })}
                  </CommandGroup>
        ...
        <Select value={sortBy} onValueChange={onSortChange}>
          ...
          <SelectContent>
            ...
            <SelectItem value="context_length:desc">上下文长度 (降序)</SelectItem>
            <SelectItem value="context_length:asc">上下文长度 (升序)</SelectItem>
        ...
        {(selectedCompanies.length > 0 || selectedProviders.length > 0) && (
          <div className="mt-4 flex flex-wrap items-center gap-2">
            {selectedCompanies.map(company => (
              <Badge key={company} variant="secondary" className="pl-2 text-sm px-3 py-1 bg-blue-100 dark:bg-blue-900">
                {company}
                <button
            ...
            {selectedProviders.map(provider => (
              <Badge key={provider} variant="secondary" className="pl-2 text-sm px-3 py-1 bg-green-100 dark:bg-green-900">
                {provider}
                <button
