...
export function ModelCard({ model, isSelected, onSelect, onCompanyClick, isCompanySelected, onProviderClick, isProviderSelected }: ModelCardProps) {
      ...
      <div className="p-4">
        <div className="flex justify-between items-start">
          <div className="flex items-center space-x-2">
            <Checkbox
              checked={isSelected}
              onCheckedChange={(checked) => onSelect(model.id, checked as boolean)}
            />
            <div>
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  {model.icon_url ? (
                    <img src={model.icon_url} alt={model.provider_display_name} className="mr-1 h-4 w-4 rounded-sm" />
                  ) : (
          ...
          <div className="flex flex-wrap items-center gap-1 px-1 py-1 bg-muted/50 rounded-md border border-muted">
            ...
            <div className="flex flex-wrap gap-1">
              {model.input_modalities.map((modality) => (
                <span key={`input-${modality}`} className="inline-flex items-center px-1 py-0.5 rounded-sm text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">
            ...
            <div className="flex flex-wrap gap-1">
              {model.output_modalities.map((modality) => (
                <span key={`output-${modality}`} className="inline-flex items-center px-1 py-0.5 rounded-sm text-xs font-medium bg-green-50 text-green-700 border border-green-200">
