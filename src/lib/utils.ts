...
export function transformModels(models: OpenRouterModel[]): Model[] {
  return models.map(model => {
    ...
    const company_name = nameParts.length > 1 ? nameParts[0].trim() : (model.author || '').trim();
    const name = nameParts.length > 1 ? nameParts.slice(1).join(':').trim() : model.name.trim();
    ...
    return {
      ...
      icon_url: icon_url.startsWith('http') ? icon_url : `https://openrouter.ai${icon_url}`
    };
  });
}
 ...
 * @returns 包含名称和数量的列表
 */
export function generateFilterList(models: Model[], groupByField: 'company_name' | 'provider_name') {
